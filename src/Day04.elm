...
partOne : List Passport -> Int
partOne =
    List.filter (isValid partOneValidation) >> List.length


partTwo : List Passport -> Int
partTwo =
    List.filter (isValid partTwoValidation) >> List.length


isValid : (Passport -> String -> Bool) -> Passport -> Bool
isValid fn passport =
    List.all (fn passport) [ "byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid" ]


partOneValidation : Passport -> String -> Bool
partOneValidation passport field =
    case Dict.get field passport of
        Just _ ->
            True
...
partTwoValidation : Passport -> String -> Bool
partTwoValidation passport field =
    case Dict.get field passport of
        Just x ->
            validationRule field x

        Nothing ->
            False


validationRule : String -> String -> Bool
validationRule k v =
    case k of
        "byr" ->
            inRangeBetween 1920 2002 v

        "iyr" ->
            inRangeBetween 2010 2020 v

        "eyr" ->
            inRangeBetween 2020 2030 v

        "hgt" ->
            case String.right 2 v of
                "cm" ->
                    inRangeBetween 150 193 (String.dropRight 2 v)

                "in" ->
                    inRangeBetween 59 76 (String.dropRight 2 v)
        ...
        "ecl" ->
            List.member v [ "amb", "blu", "brn", "gry", "grn", "hzl", "oth" ]
...
inRangeBetween : Int -> Int -> String -> Bool
inRangeBetween low high x =
    String.toInt x
...
passports =
    linesToList input
        |> List.map (String.words >> List.foldl parseField Dict.empty)


parseField : String -> Passport -> Passport
parseField x acc =
