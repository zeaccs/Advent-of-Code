...
linesToList : String -> List (Result (List DeadEnd) PasswardWithPolicy)
linesToList =
    String.lines >> List.map (run parser)
...
partOne : String -> Int
partOne =
    linesToList >> List.foldl partOneFolding 0


partTwo : String -> Int
partTwo =
    linesToList >> List.foldl partTwoFolding 0


partOneFolding : Result (List DeadEnd) PasswardWithPolicy -> Int -> Int
partOneFolding x acc =
    case x of
        Ok v ->
            let
                count =
                    List.length (String.indexes v.char v.password)
            in
            if count >= v.min && count <= v.max then
                acc + 1
...
partTwoFolding : Result (List DeadEnd) PasswardWithPolicy -> Int -> Int
partTwoFolding x acc =
    case x of
        Ok v ->
            let
                indexes =
                    String.indexes v.char v.password

                isAtFirst =
                    List.member (v.min - 1) indexes

                isAtSecond =
                    List.member (v.max - 1) indexes
            in
            if xor isAtFirst isAtSecond then
                acc + 1
...
parser : Parser PasswardWithPolicy
parser =
    succeed PasswardWithPolicy
        |= int
        |. symbol "-"
        |= int
        |. spaces
